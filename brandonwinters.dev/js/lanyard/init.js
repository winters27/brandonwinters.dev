...
const draw = data => {
    ...
    acts.forEach(act => {
        ...
        if(act.type === 4) {
            ...
            if (data.spotify && spotifyActivity) {
                const progressBar = document.getElementById('spotify-progress-bar');
                const songStart = new Date(data.spotify.timestamps.start).getTime();
                const songEnd = new Date(data.spotify.timestamps.end).getTime();
                const currentTime = Date.now();
        
                const songDuration = songEnd - songStart;
                const currentProgress = currentTime - songStart;
            ...
            if(isSpotify) {
                // Create a clickable link for the song title
                const spotifyUrl = `https://open.spotify.com/track/${data.spotify.track_id}`;
                nameSpan.innerHTML = `<a href="${spotifyUrl}" target="_blank" class="gradient-link">${data.spotify.song}</a>`;
